...
var controller = {
    ...
    updateProject: function(req, res){
        ...
        Project.findByIdAndUpdate(projectId, update, {new:true}, (err, projectUpdated) => {
            if(err) return res.status(500).send({message: 'Error al encontrar el documento.'});

            if(!projectUpdated) return res.status(404).send({message: 'No existe el documento que se quiere actualizar.'});
    ...
    deleteProject: function(req, res){
        ...
        Project.findByIdAndRemove(projectId, (err, projectRemoved) => {
            if(err) return res.status(500).send({message: 'Error al eliminar el dato.'});

            if(!projectRemoved) return res.status(404).send({message: 'El documento no existe.'});
